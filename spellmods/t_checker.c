/* t spellmod 
   ...
   syntax: t_checker <threshold> <t list> <valkuil-inst>
...
int main(int argc, char *argv[])
{
  ...
  if (DEBUG)
    fprintf(stderr,"read %d t triggers from %s\n",
	    nrddt,argv[2]);
  ...
  // tell the Timbl server to use the t base
  sock_puts(sock,"base t\n");
	    ...
	    (strcmp(feats[mid],"<end>")==0)))
	{
	  ...
	  if (ddtmatch)
	      ...
	      if (part!=NULL)
		{
		  ...
		  if (feats[mid][strlen(feats[mid])-1]!='t')
		    other='n';
		  else
		    other='t';
