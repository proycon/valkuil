/* d-dt spellmod 
   ...
   syntax: d-dt_checker <threshold> <d-dt list> <valkuil-inst>
...
int main(int argc, char *argv[])
{
  ...
  if ((threshold<0.5)||
      (threshold>1.0))
    {
      fprintf(stderr,"[d-dt_checker] ERROR: threshold value not between 0.5 and 1.0\n");
      exit(1);
    }
  ...
  if (DEBUG)
    fprintf(stderr,"read %d d-dt triggers from %s\n",
	    nrddt,argv[2]);
