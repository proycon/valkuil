/* tte-tten spellmod 
   ...
   syntax: tte-tten_checker <threshold> <tte-tten list> <valkuil-inst>
...
int main(int argc, char *argv[])
{
  ...
  if (DEBUG)
    fprintf(stderr,"read %d tte-tten triggers from %s\n",
	    nrddt,argv[2]);
  ...
  // tell the Timbl server to use the tte-tten base
  sock_puts(sock,"base tte-tten\n");
			  ...
			  if (category[0]=='n')
			    {
			      fprintf(stdout,"%sn",
				      feats[mid]);
			      fprintf(stderr,"correcting %s to %sn\n",
				      feats[mid],feats[mid]);
			    }
			  else
			    {
